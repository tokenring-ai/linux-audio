...
import {LinuxAudioAccountSchema, LinuxAudioConfigSchema, type LinuxAudioAccount} from "./schema.ts";

const packageConfigSchema = z.object({
  linuxAudio: LinuxAudioConfigSchema.prefault({accounts: {}}),
});

function addAccountsFromEnv(accounts: Record<string, Partial<LinuxAudioAccount>>) {
  ...
  install(app, config) {
    ...
    app.waitForService(AudioService, audioService => {
      for (const [name, account] of Object.entries(config.linuxAudio.accounts)) {
        audioService.registerProvider(name, new LinuxAudioProvider({
          type: "linux",
